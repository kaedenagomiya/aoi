...
class NixTokenizerEN:
    ...
    def __call__(
        ...
        tokens = [
            self._intersperse(self._safe_vocab_lookup(phoneme), 0) 
    ...
    def _safe_vocab_lookup(self, phoneme):
        """Safe lookup in vocab_dict to handle missing phoneme entries."""
        return [self.vocab_dict.get(p, self.vocab_dict.get('<unk>', 0)) for p in phoneme]
