...
class TimeFreqKernelConv2d(BaseModule):
    ...
    def forward(self, x):
        #x = self.conv1(x)
        x = x + self.tfkconv_f(x) + self.tfkconv_t(x) # for tfkful_plus
        #x = x * torch.sigmoid(self.tfkconv_f(x) + self.tfkconv_t(x)) # for tfkful_mask
        x = self.pointwise(x)
        return x
...
class LinearAttention(BaseModule):
    ...
    def __init__(self, dim, heads=4, dim_head=32):
        ...
        #self.to_q and k and v = torch.nn.Conv2d(dim, self.hidden_dim, 1, bias=False)
        #self.to_out = torch.nn.Conv2d(self.hidden_dim, dim, 1)
        self.to_q = TimeFreqKernelConv2d(dim, self.hidden_dim, tf_kernel=(3,3), padding=1, bias=False)
        self.to_k = TimeFreqKernelConv2d(dim, self.hidden_dim, tf_kernel=(3,3), padding=1, bias=False)
        self.to_v = TimeFreqKernelConv2d(dim, self.hidden_dim, tf_kernel=(3,3), padding=1, bias=False)
        self.to_out = TimeFreqKernelConv2d(self.hidden_dim, dim, tf_kernel=(3,3), padding=1)
